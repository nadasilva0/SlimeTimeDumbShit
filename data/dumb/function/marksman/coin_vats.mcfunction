## Sets cooldowns
scoreboard players set @s genericcooldown 90
...
execute at @s[tag=marksman] run summon slime ~ ~1.5 ~ {CustomNameVisible:0b,DeathLootTable:"na",Motion:[0.0,0.6,0.0],Tags:["coin"],CustomName:'"coin"',attributes:[{id:"minecraft:gravity",base:0.02}]}
scoreboard players set @n[type=slime,tag=coin] vatsactive 1
...
execute as @e[tag=coin] store result entity @s Motion[0] double 0.00066 run scoreboard players get @s MotionX
#execute as @e[tag=coin] store result entity @s Motion[1] double 0.001 run scoreboard players get @s MotionY
execute as @e[tag=coin] store result entity @s Motion[2] double 0.00066 run scoreboard players get @s MotionZ

## Plays sound
playsound minecraft:custom.coinflip master @a ~ ~ ~ 1 1

## Does weapon cooldown stuff
scoreboard players set .coin projdist 75
