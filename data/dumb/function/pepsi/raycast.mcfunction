...
execute as @e[tag=!raycasting,type=!#slimelib:not_mob,dx=0] positioned ~-0.50 ~-0.50 ~-0.50 if entity @s[dx=0] positioned ~0.50 ~0.50 ~0.50 run function dumb:pepsi/collide
execute positioned ^ ^ ^.5 unless block ^ ^ ^ #slimelib:raycast_pass run function dumb:pepsi/collide_ground
...
execute if score .distance tf_rc matches ..5000 positioned ^ ^ ^0.5 rotated ~ ~0.01 if block ~ ~ ~ #slimelib:raycast_pass run function dumb:pepsi/raycast